...
# Архітектура системи
Платформа побудована з використанням предметно-орієнтованого проектування (DDD). 
Предметно-орієнтоване проектування – це підхід до моделювання складного об'єктно-орієнтованого програмного забезпечення. 
Його переваги полягають в концентрації основної уваги  на предметній області та створенні програмних моделей, які відображають глибоке розуміння предметної області.

Основною частиною сервісу, є доменний рівень. Цей рівень розділений на три зв’язні контексти:


### • _Контекст благодійників_ 


### • _Контекст медичних закладів_ 


### • _Контекст благодійних проектів_


Кожен контекст містить:


• _доменні сутності_ - об'єкти, які описують сутність з моделі предметної області, і реалізовують бізнес-логіку системи. Також варто зазначити, що однією з основних характеристик сутності є наявність ідентифікатор. Для прикладу в контексті благодійних проектів є сутність CharityProject. Ця сутність містить всі атрибути та методи, які описують благодійний проект з точки зору бізнес логіки. 

• _об’єкти значення_ - об'єкти, які містять атрибути, але не мають концептуальної ідентичності. Такі об’єкти повинні розглядатися як незмінні об'єкти. Прикладом об’єкту значення є DonationRecord, який описує благодійний внесок, але без сутності CharityProject, цей об’єкт не може існувати і варто зазначити, що DonationRecord залишається незмінним після його створення. 


• _доменні помилки_ - об’єкти, які описують бізнес помилки, які генеруються в системі. Для прикладу помилка InvalidCharityProjectState виникає, коли благодійник намагається підтримати проект, який ще не є затвердженим.


• _доменні події_ - об’єкти, які описують бізнес події, які генеруються доменним рівнем. Прикладом доменної події є подія створення благодійного проекту - CharityProjectCreated, та подія затвердження благодійного проекту - CharityProjectApproved.


Використовуючи предметно-орієнтоване проектування, я чітко відокремив бізнес логіку системи від її інфраструктури, та зміг написати логіку максимально наближено до реальних бізнес процесів. Також DDD дав мені можливість оптимізувати процес написання тестів, адже для тестування бізнес логіки системи,  мені достатньо протестувати лише доменний рівень. Також варто зазначити що доменний рівень є достатньо гнучким та легко піддається розширенню та модифікуванню.
